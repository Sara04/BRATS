...
class PatchExtractorBRATS(object):
    ...
    def __init__(self, w=240, h=240, d=155,
                 ...
                 td_th_1=20, td_th_2=256, lpm_d=2, spm_d=1,
                 augment_train=True):
    ...
    def _modality_patches(self, scan, m, volume, b, mode):
        ...
        if mode == 'train' and self.augment_train:
            ...
            b_ = [b_[0] + sh_1, b_[1] + sh_1, b_[2] + sh_2, b_[3] + sh_2]
        ...
        if b_[1] > self.h:
            d_[1], b_[1] = [self.lp_h - (b_[1] - self.h), self.h]
        if b_[3] > self.w:
            d_[3] = [self.lp_w - (b_[3] - self.w), self.w]
    ...
    def _class_patches(self, db, scan, volumes, mask, pp, seg, mode):
        lpc = np.zeros((seg.patch_per_scan, self.lp_h * self.lp_w * self.lp_d))
        spc = np.zeros((seg.patch_per_scan, self.sp_h * self.sp_w * self.sp_d))

        n_available = len(mask[0])
        if n_available:
            n_select = np.min([seg.patch_per_scan, n_available])
            ...
            for s_idx, s in enumerate(select):
                ...
                lp[:, :, self.lpm_d * db.n_ms: (self.lpm_d * db.n_ms + 3)] =\
    ...
    def _scan_patches(self, scan, db, pp, seg, mode):
        ps = {'region_1': {}, 'region_2': {}}
        volumes = scan.load_volumes(db, normalized=True)
        ...
        for c in range(2):
            ...
            ps['region_2'][c]['l_patch'], ps['region_2'][c]['s_patch'] =\
                self._class_patches(db, scan, volumes, mask_2, pp, seg, mode)
        return ps
    ...
    def extract_data(self, db, pp, seg, exp_out, mode='train'):
        ...
        """
        data = self._allocate_data_memory(db, seg)
        selected_scans = self._select_scans_randomly(db, seg, mode)
        ...
        for s_idx, s in enumerate(selected_scans):
            data_s = self._scan_patches(data_dict, db, pp, seg, mode)
            for i in db.classes:
        ...
        data_r1, data_r2 = self._shuffle_and_select_data(data, c, db)
