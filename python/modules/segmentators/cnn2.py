...
class CnnBRATS2(object):
    """Segmentation class."""

    def __init__(self, lp_w=45, lp_h=45, lp_d=11, sp_w=17, sp_h=17, sp_d=4):
        ...
        with tf.variable_scope('l_patches'):
            ...
            with tf.variable_scope('layer_3'):
                ...
                lp_h_conv3_r2 = tf.nn.relu(_conv2d(lp_h_pool2_r2, lp_w_conv3) +
                                           lp_b_conv3)
                lp_h_conv3_flat_r1 =\
                    tf.reshape(lp_h_conv3_r1, [-1, 4 * 4 * 128])
                lp_h_conv3_flat_r2 =\
                    tf.reshape(lp_h_conv3_r2, [-1, 4 * 4 * 128])
            with tf.variable_scope('layer_4'):
                lp_w_fcn1 = _weight_variable([4 * 4 * 128, 128])
                lp_b_fcn1 = _bias_variable([128])
                lp_h_fcn1_r1 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(lp_h_conv3_flat_r1, lp_w_fcn1) +
                                             lp_b_fcn1), self.keep_prob)
                lp_h_fcn1_r2 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(lp_h_conv3_flat_r2, lp_w_fcn1) +
                                             lp_b_fcn1), self.keep_prob)
            with tf.variable_scope('layer_5'):
                lp_w_fcn2 = _weight_variable([128, 32])
                lp_b_fcn2 = _bias_variable([32])
                lp_h_fcn2_r1 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(lp_h_fcn1_r1, lp_w_fcn2) +
                                             lp_b_fcn2), self.keep_prob)
                lp_h_fcn2_r2 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(lp_h_fcn1_r2, lp_w_fcn2) +
                                             lp_b_fcn2), self.keep_prob)
        with tf.variable_scope('s_patches'):
            with tf.variable_scope('layer_1'):
                sp_w_conv1 = _weight_variable([5, 5, 4, 16])
                ...
                sp_h_conv1_r2 = tf.nn.relu(_conv2d(sp_imgs_r2, sp_w_conv1) +
                                           sp_b_conv1)
            with tf.variable_scope('layer_2'):
                sp_w_conv2 = _weight_variable([5, 5, 16, 32])
                ...
                sp_h_conv2_r2 = tf.nn.relu(_conv2d(sp_h_conv1_r2, sp_w_conv2) +
                                           sp_b_conv2)
            with tf.variable_scope('layer_3'):
                sp_w_conv3 = _weight_variable([5, 5, 32, 64])
                sp_b_conv3 = _bias_variable([64])
                sp_h_conv3_r1 = tf.nn.relu(_conv2d(sp_h_conv2_r1, sp_w_conv3) +
                                           sp_b_conv3)
                sp_h_conv3_r2 = tf.nn.relu(_conv2d(sp_h_conv2_r2, sp_w_conv3) +
                                           sp_b_conv3)

                sp_h_conv3_flat_r1 =\
                    tf.reshape(sp_h_conv3_r1, [-1, 5 * 5 * 64])
                sp_h_conv3_flat_r2 =\
                    tf.reshape(sp_h_conv3_r2, [-1, 5 * 5 * 64])
            with tf.variable_scope('layer_4'):
                sp_w_fcn1 = _weight_variable([5 * 5 * 64, 128])
                sp_b_fcn1 = _bias_variable([128])
                sp_h_fcn1_r1 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(sp_h_conv3_flat_r1, sp_w_fcn1) +
                                  sp_b_fcn1), self.keep_prob)
                sp_h_fcn1_r2 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(sp_h_conv3_flat_r2, sp_w_fcn1) +
                                  sp_b_fcn1), self.keep_prob)
            with tf.variable_scope('layer_5'):
                sp_w_fcn2 = _weight_variable([128, 32])
                sp_b_fcn2 = _bias_variable([32])
                sp_h_fcn2_r1 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(sp_h_fcn1_r1, sp_w_fcn2) +
                                             sp_b_fcn2), self.keep_prob)
                sp_h_fcn2_r2 =\
                    tf.nn.dropout(tf.nn.relu(tf.matmul(sp_h_fcn1_r2, sp_w_fcn2) +
                                             sp_b_fcn2), self.keep_prob)
        ...
        cross_entropy =\
            0.25 * tf.reduce_mean(-tf.reduce_sum(self.y_gt_r1 *
                                                 tf.log(mp_h_fcn1_r1),
                                                 reduction_indices=[1])) +\
            0.75 * tf.reduce_mean(-tf.reduce_sum(self.y_gt_r2 *
                                                 tf.log(mp_h_fcn1_r2),
                                                 reduction_indices=[1]))
        self.train_step = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(cross_entropy)
    ...
    def train(self, lr, train_data_r1, train_data_r2):
